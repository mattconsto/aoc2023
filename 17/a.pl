...
while (my %node = %{$queue->pop || {}}) {
    ...
    if ($node{y} > 0 && (!$node{dir} || ($node{dir} ne 's' && ($node{dir} ne 'n' || $node{len} < 3)))) {
        $queue->insert({
            path => $node{path} . '^',
            dir  => 'n',
    ...
    if ($node{x} < length($map[$node{y}]) - 1 && (!$node{dir} || ($node{dir} ne 'w' && ($node{dir} ne 'e' || $node{len} < 3)))) {
        $queue->insert({
            path => $node{path} . '>',
            dir  => 'e',
    ...
    if ($node{y} < scalar @map - 1 && (!$node{dir} || ($node{dir} ne 'n' && ($node{dir} ne 's' || $node{len} < 3)))) {
        $queue->insert({
            path => $node{path} . 'v',
            dir  => 's',
    ...
    if ($node{x} > 0 && (!$node{dir} || ($node{dir} ne 'e' && ($node{dir} ne 'w' || $node{len} < 3)))) {
        $queue->insert({
            path => $node{path} . '<',
            dir  => 'w',
